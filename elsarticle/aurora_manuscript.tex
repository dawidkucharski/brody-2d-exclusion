...
\textbf{This work.} This study tests the hypothesis that the Brody exponent
$\beta$, when properly calibrated against the 2D complete-spatial-randomness
(CSR) baseline, provides an empirically robust measure of
short-range exclusion strength in 2D spatial point processes. The
investigation proceeds in three stages. First, synthetic hard-core and
soft-core point processes are used to establish the empirical
$\beta$--$r_{\text{excl}}/\langle\text{NN}\rangle$ mapping and to
determine the correct 2D CSR baseline (which is $\beta \approx 0.96$,
not $\beta = 0$ as inherited from 1D). Second, this calibrated $\beta$
scale is applied to three physically unrelated domains: (i)~58 manufactured surface textures (10 materials, 10
manufacturing processes, focus-variation microscopy), (ii)~2D binary embeddings of the primes and related
arithmetic sequences, and (iii)~phase-extracted
interferometric profilometry of a certified roundness standard
(JENOPTIK FN~111, 24\,000 frames, 5 continuous $400^\circ$ rotations).
PSI data were included because they provide an independently generated
spatial point process arising from optical phase reconstruction rather
than material manufacturing, offering a third generative domain
unrelated to either plastic deformation or number theory.
Density-thinning experiments are performed to isolate genuine exclusion
structure from trivial density effects. Third, for binary patterns where peak detection is not applicable,
complementary spatial descriptors are developed and shown to probe
orthogonal aspects of spatial structure (Supplementary Information).
The central finding is that all three domains exhibit $\beta$ values
significantly exceeding their density-matched CSR baselines,
establishing genuine exclusion beyond density effects, but that
absolute $\beta$ values are density-dependent. Cross-domain consistency
in the $\beta$--$r_{\text{excl}}$ mapping supports the interpretation
of $\beta$ as an empirically robust exclusion measure. However, the
$\beta$--$r_{\text{excl}}$ relation is calibration-based rather than
derived from first principles; it is an empirical mapping established
through synthetic point-process experiments, not a theoretical
necessity. The density dependence and embedding sensitivity
indicate that shared exclusion signatures---rather than a single
universal mechanism---characterise the three domains.
...
\textbf{Roadmap.} The Results are organised around a single question:
does $\beta$ quantify exclusion strength? Sections~3.1--3.3
present $\beta$ and spatial benchmarks for manufactured surfaces.
Section~3.4 establishes the synthetic $\beta$--$r_{\text{excl}}$
calibration and the 2D CSR baseline---this is the interpretive key
for all data. Section~3.5 presents $g_2(r)$ as an internal consistency
check. Section~3.6 presents density-thinning controls that isolate
genuine exclusion from density effects. Sections~3.7--3.8 briefly
summarise complementary $D_2$ and Banach descriptors for binary
patterns, with full detail provided in Supplementary Information~(SI).
Readers focused on the main $\beta$-exclusion
thesis may proceed directly from Section~3.6 to the Discussion~(Section~4).
...
nearest-neighbour index (NNI = observed/expected NN distance
under complete spatial randomness), Ripley's $K$ normalised by
the CSR expectation at the mean NN distance, Voronoi cell area
variance, and the pair correlation function $g(r)$. The Brody
...
fixed continuum (Poisson at 0, strong repulsion at $>1$),
whereas NNI and Ripley's $K$ are scale-dependent and require
reference to CSR expectations.
...
Sixth, the arithmetic surface analysis is limited to $N = 10^5$ for
...
All analysis code, surface metrology reports, and processed data are
available at the project repository. The \texttt{aurora\_lab} Python
