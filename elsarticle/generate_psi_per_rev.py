...
def main():
    ...
    # === PER-REVOLUTION β COMPUTATION ===
    print("\nPer-revolution Brody analysis...")
    beta_per_rev = []; n_peaks_per_rev = []; rms_per_rev = []; pv_per_rev = []
    for rev_idx in range(len(ath)):
        ts_rev = ath[rev_idx]; hs_rev = ah[rev_idx]
        si_rev = np.argsort(ts_rev); tf_rev = ts_rev[si_rev]; hf_rev = hs_rev[si_rev]*1000
        rms_r = np.std(hf_rev); pv_r = np.max(hf_rev)-np.min(hf_rev)
        prom_nm_r = PKPROM*(np.max(hf_rev)-np.min(hf_rev))
        pi_r, _ = signal.find_peaks(hf_rev, prominence=prom_nm_r, distance=5)
        npk_r = len(pi_r)
        if npk_r < 5:
            pi_r, _ = signal.find_peaks(hf_rev, prominence=prom_nm_r/3, distance=5)
            npk_r = len(pi_r)
        if npk_r >= 5:
            pa_r = tf_rev[pi_r]
            pas_r = np.sort(pa_r); ang_sp_r = np.diff(pas_r)
            ans_r = ang_sp_r/np.mean(ang_sp_r)
            fit_r = fit_brody(ans_r)
            beta_r = fit_r['beta']
        else:
            beta_r = np.nan
        beta_per_rev.append(float(beta_r) if not np.isnan(beta_r) else None)
        n_peaks_per_rev.append(int(npk_r))
        rms_per_rev.append(float(rms_r)); pv_per_rev.append(float(pv_r))
        status = "OK" if npk_r >= 5 else "LOW"
        print(f"  Rev {rev_idx+1}: β={beta_r:.2f}[{fit_r['ci_low']:.2f},{fit_r['ci_high']:.2f}]"
              f" peaks={npk_r} RMS={rms_r:.0f}nm PV={pv_r:.0f}nm [{status}]")
    valid_betas = [b for b in beta_per_rev if b is not None]
    if valid_betas:
        print(f"\n  Mean β = {np.mean(valid_betas):.2f} ± {np.std(valid_betas):.2f} (n={len(valid_betas)} revolutions)")
        print(f"  β range: [{np.min(valid_betas):.2f}, {np.max(valid_betas):.2f}]")
    # Save per-revolution results
    per_rev_results = {
        'beta_per_rev': beta_per_rev,
        'n_peaks_per_rev': n_peaks_per_rev,
        'rms_nm_per_rev': rms_per_rev,
        'pv_nm_per_rev': pv_per_rev,
        'mean_beta': float(np.mean(valid_betas)) if valid_betas else None,
        'std_beta': float(np.std(valid_betas)) if valid_betas else None,
        'n_valid_revolutions': len(valid_betas),
        'total_revolutions': NREV
    }
    with open(f'{OUTDIR}/psi_per_rev_results.json', 'w') as f:
        json.dump(per_rev_results, f, indent=2)
    print(f"  Saved: psi_per_rev_results.json")
    # === END PER-REVOLUTION ===
